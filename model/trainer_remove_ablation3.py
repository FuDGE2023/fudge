...
class Trainer(nn.Module):
    def __init__(self, params):
        ...
        self.gnn_aux = EncoderGIN(self.in_feat, self.h_dim_encoder, self.z_dim_encoder).to(self.device)
        ...
        self.optim_gnn_aux = torch.optim.Adam(self.gnn_aux.parameters(), lr=self.learning_rate)
    ...
    def _train(self, snapshot_train):
        ...
        for idx, snapshot_seq in tqdm(enumerate(snapshot_train), total=len(snapshot_train)):
            ...
            h_last, z_last = self.gnn_aux(edge_index, x, edge_index_t)
        ...
        try:
            for epoch in range(self.params['n_epochs']):
                ...
                if best_loss > val_loss:
                    ...
                    torch.save(self.gnn_aux.state_dict(), self.params['gnn_aux'])
    ...
    def generate_graph_rnd_alternative(self, history, num_generation, models_event, models_add_source, models_add_dest):
        gnn_history_event, rnn_history_event, event_network, gnn_aux = models_event
        gnn_history_source, rnn_history_source, net_source = models_add_source
        gnn_history_dest, rnn_history_dest, net_dest = models_add_dest
        ...
        with torch.no_grad():
            ...
            while count < num_generation:
                ...
                if event == 0:
                    ...
                    h_last, z_last = gnn_aux(edge_index, x, edge_index_t)
