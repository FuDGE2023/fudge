...
class NAT(torch.nn.Module):
  ...
  def position_bits(self, bs, hop):
    ...
    return torch.ones(bs * self.num_neighbors[hop], device=self.device, dtype=int) << hop
  ...
  def contrast(self, src_l_cut, tgt_l_cut, bad_l_cut, cut_time_l, e_idx_l=None, test=False):
    ...
    pos_raw = pos_raw.to(dtype=torch.int32)
    ...
    tgt_pos_raw = tgt_pos_raw.to(dtype=torch.float32)
    bad_pos_raw = bad_pos_raw.to(dtype=torch.float32)
