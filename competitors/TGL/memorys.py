...
class MailBox():
    ...
    def update_memory(self, nid, memory, root_nodes, ts, neg_samples=1, generation=False, idx_nodes=[]):
        if nid is None:
            return
        num_true_src_dst = root_nodes.shape[0] // (neg_samples + 2) * 2
        with torch.no_grad():
            if generation:
                nid = nid[idx_nodes].to(self.device)
                memory = memory[idx_nodes].to(self.device)
                ts = ts[idx_nodes].to(self.device)
            else:
    ...
    def update_mailbox(self, nid, memory, root_nodes, ts, edge_feats, block, neg_samples=1, generation=False,
                       sources=[], destinations=[], idx_sources=[], idx_destinations=[]):
        with torch.no_grad():
            ...
            if self.memory_param['deliver_to'] == 'self':
                if generation:
                    src = torch.from_numpy(sources).to(self.device)
                    dst = torch.from_numpy(destinations).to(self.device)
                    mem_src = memory[idx_sources]
                    mem_dst = memory[idx_destinations]
                else:
            ...
            # APAN
                ...
                if self.dim_edge_feat > 0:
                    ...
                    print(src_mail.shape)
                    dst_mail = torch.cat([mem_dst, mem_src, edge_feats], dim=1)
                else:
                    src_mail = torch.cat([mem_src, mem_dst], dim=1)
                    dst_mail = torch.cat([mem_dst, mem_src], dim=1)
                mail = torch.cat([src_mail, dst_mail], dim=0)
                # print(mail.reshape(-1, src_mail.shape[1]).shape)
                # print(len(block.edges()))
                # print(block.edges()[0])
                print(block.edges()[0].shape)
                # print(block.edges()[1].shape)
                # print(block.edges()[0].long())
